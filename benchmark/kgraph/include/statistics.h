// statistics.h - Graph statistics for KGraph benchmarks
#pragma once
...
namespace kgraph::benchmark::statistics {
...
static void GenRandom(RNG& rng, uint32_t* addr, uint32_t size, uint32_t N) {
    if (N == size) {
        for (uint32_t i = 0; i < size; ++i) addr[i] = i;
        return;
    }
    for (uint32_t i = 0; i < size; ++i) addr[i] = rng() % (N - size);
    std::sort(addr, addr + size);
    for (uint32_t i = 1; i < size; ++i) {
        if (addr[i] <= addr[i - 1]) addr[i] = addr[i - 1] + 1;
    }
    uint32_t off = rng() % N;
    for (uint32_t i = 0; i < size; ++i) addr[i] = (addr[i] + off) % N;
}
...
static uint32_t compute_reachability_count(std::vector<std::vector<uint32_t>>& graph) {
    ...
    std::vector<uint32_t> random(params.P);
    GenRandom(rng, random.data(), static_cast<uint32_t>(random.size()), static_cast<uint32_t>(graph_size));

    std::vector<bool> visited(graph_size);
    std::vector<uint32_t> frontier;
    for (uint32_t s : random) {
        visited[s] = true;
...
struct ReachCache {
    ...
    void alias(uint32_t v, uint32_t idx) {
        mapping[v].store(idx, std::memory_order_release);
    }
...
uint32_t compute_reach_for_vertex(
    size_t start,
    std::vector<std::vector<uint32_t>>& graph,
    ReachCache& cache,
    BestTracker& best_global)
{
...
struct BestSingleThread {
    ...
    bool try_promote(uint32_t r) {
        if (r > value) { value = r; return true; }
        return false;
    }
};
...
static uint32_t compute_avg_reach(std::vector<std::vector<uint32_t>>& graph, size_t num_threads = 1) {
    ...
    parallel_for(size_t{0}, n, num_threads, [&](size_t v, size_t tid) {
        sums[tid] += compute_reach_for_vertex(v, graph, cache, best);
    });
...
static void compute_stats(kgraph::KGraph* graph, const char* top_list_file) {
    log("\n--- Graph Analysis ---\n");

    auto compact_graph = graph->getCompactGraph();
    ...
    for (size_t n = 0; n < graph_size; n++) {
        size_t deg = compact_graph[n].size();
    ...
    for (size_t n = 0; n < graph_size; n++) {
        for (uint32_t neighbor : compact_graph[n]) {
    ...
    uint32_t search_reach = compute_reachability_count(compact_graph);

    log("Computing exploration reachability...\n");
    uint32_t explore_reach = compute_avg_reach(compact_graph);
...
}  // namespace kgraph::benchmark::stats
