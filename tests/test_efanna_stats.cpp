...
static void compute_stats(const char* graph_file, const uint32_t feature_dims, const char* top_list_file) {
    ...
    auto init_index = efanna2e::IndexRandom(feature_dims, top_list_count);
    auto index = efanna2e::IndexGraph(feature_dims, top_list_count, efanna2e::L2, (efanna2e::Index*)(&init_index));
    ...
    float avg_edge_count = 0;
    {
        ...
        for (uint32_t n = 0; n < graph_size; n++) {
            ...
            if(top_list_dims < edges_per_node) {
                std::cerr << "TopList for vertex " << n << " is not long enough, has " << top_list_dims << " elements expected " << edges_per_node << std::endl;
...
int main(int argc, char** argv) {
    ...
    // // ----------------------------------------- SIFT1M ------------------------------------------------
    // const auto dims = 128;
    // auto top_list_file  = R"(e:/Data/Feature/SIFT1M/SIFT1M/sift_base_top1000.ivecs)";

    // // GQ 0.9396, avg degree 200.0, min_out 200, max_out 200, min_in 1, max_in 2622, source nodes 0, search reachability count 999940, exploration avg reach 999940.00, node count 1000000
    // // auto efanna_file   = R"(c:/Data/Feature/SIFT1M/efanna/sift_200nn.graph)"; 

    // // GQ 0.9523, avg degree 200.0, min_out 200, max_out 200, min_in 0, max_in 2646, source nodes 66, search reachability count 999926, exploration avg reach 999926.00, node count 1000000
    // // auto efanna_file   = R"(c:/Data/Feature/SIFT1M/efanna/efanna_K200_L200_It12_S10_R100.efa)";     
    ...
    // ----------------------------------------- Glove ------------------------------------------------
    // const auto dims = 100;
    // auto top_list_file  = R"(e:/Data/Feature/GloVe/glove-100/glove-100_base_top1000.ivecs)";
    // // auto efanna_file   = R"(e:/Data/Feature/GloVe/efanna/glove-100_400nn.knng)";                 // Seed Reachablity 1012901 of 1183514 vertices
    ...
    // GQ 0.9985, avg degree 40.0, min_out 40, max_out 40, min_in 0, max_in 1312, source nodes 2829, search reachability count 87362, exploration avg reach 84546.86, node count 94987
    auto efanna_file    = R"(e:/Data/Feature/Enron/efanna/K40_L140_It5_S35_R150_(nTrees4_mLevel8).efa)";

    compute_stats(efanna_file, dims, top_list_file);
